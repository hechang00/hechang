...
<template :options="" props="" value="" separator="/"
          popper-class=""
          placeholder="请选择123"
          disabled=false
          clearable=false
          ...
          :disabled=""
          @change="change"
          @active-item-change="active-item-change"
          @blur="blur"
          @focus="focus">
  <div
    ref="reference"
...
import Menu from "./src/cascader-menu";
import Node from "./src/cascader-node";
import Panel from "./src/cascader-panel";
...
export default {
    ...
    checkedValue(val) {
      ...
      if (!isEqual(val, value) || isUndefined(value)) {
        ...
        for (let current of val) {
          selected = this.localOptions.filter(v => v.value === current)[0];
          if (!selected) break;
        ...
        this.$emit("change", { value: val, children: selected });
  ...
  mounted() {
    ...
    this.getOptionList(); //从后台获取级联选择器options数据
  },
  ...
  methods: {
    getOptionList() {
      let _this = this;
      jimu.request.get({
        url: "https://app.cf.com/api/common/busiType/list",
        data: {},
        ignoreGlobalHeader: true,
        headers: {},
        timeout: 60000,
        successBlock: function(res) {
          if (res.code === 0) {
            _this.localOptions = res.data;
            _this.$emit("input", ["-1"]);
            _this.$emit("loadSuccess", res.data);
            _this = null;
          }
        }, //为undefined则返回Promise
        failBlock: function(error) {
          _this.$emit("loadFail", error);
          _this = null;
        } //为undefined则返回Promise
      });
    },
    ...
    computePresentText() {
      ...
      if (!isEmpty(checkedValue) && this.panel) {
